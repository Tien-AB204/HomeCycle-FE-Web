...
const FieldError = ({ children }) =>
  children ? <p className="mt-1 text-xs font-semibold text-error">{children}</p> : null;
...
const AgreementForm = ({ agreement, negotiationId, onSubmit, onCancel, busy }) => {
  ...
  const inputClass = "mt-1.5 w-full rounded-xl border border-border bg-background px-3.5 py-3 text-sm text-text outline-none transition focus:border-primary focus:bg-white focus:ring-4 focus:ring-primary/10";

  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <section className="rounded-2xl border border-border bg-white p-5 shadow-[0_10px_30px_rgba(23,40,48,0.05)] sm:p-6">
        <p className="text-xs font-bold uppercase tracking-[0.18em] text-primary">1. Hình thức thỏa thuận</p>
        <div className="mt-4 grid gap-3 md:grid-cols-2">
          {AGREEMENT_TYPE_OPTIONS.map((option) => (
            <label key={option.value} className={`cursor-pointer rounded-xl border p-4 transition ${values.agreementType === option.value ? "border-primary bg-primary/10" : "border-border hover:border-primary"}`}>
              ...
              <span className="mt-1 block text-xs leading-5 text-textLight">{option.description}</span>
            </label>
          ))}
        </div>
      ...
      {values.agreementType === AGREEMENT_TYPE.INSPECTION && (
        <section className="rounded-2xl border border-border bg-white p-5 shadow-[0_10px_30px_rgba(23,40,48,0.05)] sm:p-6">
          <p className="text-xs font-bold uppercase tracking-[0.18em] text-primary">2. Lịch kiểm định</p>
          <div className="mt-4 grid gap-4 md:grid-cols-2">
            <label className="text-sm font-bold text-text">
              Ngày giờ kiểm định <span className="text-error">*</span>
      ...
      <section className="rounded-2xl border border-border bg-white p-5 shadow-[0_10px_30px_rgba(23,40,48,0.05)] sm:p-6">
        <p className="text-xs font-bold uppercase tracking-[0.18em] text-primary">{values.agreementType === AGREEMENT_TYPE.INSPECTION ? "3" : "2"}. Giao nhận</p>
        <div className="mt-4 grid gap-4 md:grid-cols-2">
          <label className="text-sm font-bold text-text">
      ...
      <section className="rounded-2xl border border-border bg-white p-5 shadow-[0_10px_30px_rgba(23,40,48,0.05)] sm:p-6">
        <p className="text-xs font-bold uppercase tracking-[0.18em] text-primary">{values.agreementType === AGREEMENT_TYPE.INSPECTION ? "4" : "3"}. Điều khoản chung</p>
        <label className="mt-4 block text-sm font-bold text-text">
          Nội dung đã thống nhất <span className="text-error">*</span>
      ...
      <div className="flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
        {onCancel && <button type="button" onClick={onCancel} disabled={busy} className="rounded-lg border border-primary bg-white px-5 py-3 text-sm font-bold text-primary hover:bg-primary/10 disabled:opacity-50">Hủy chỉnh sửa</button>}
        <button type="submit" disabled={busy} className="rounded-lg bg-primary px-6 py-3 text-sm font-black text-white transition hover:bg-primary/90 disabled:cursor-not-allowed disabled:opacity-50">
